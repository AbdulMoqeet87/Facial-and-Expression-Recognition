...
print("Percentage Variance for Each Principal Component (Subject 1):")
# for i, variance in enumerate(percentage_variance):
#     print(f"PC {i + 1}: {variance:.2f}%")

# Find the indices of the first three principal components
first_three_indices = [0, 1, 2]

# Display the final images based on the first three principal components
plt.figure(figsize=(5, 5))
for i, idx in enumerate(first_three_indices):
    final_image = resultant_matrix[0].reshape(images_resized[0].shape)
    plt.subplot(2, 3, i + 1)
    plt.imshow(final_image, cmap='gray')
    plt.title(f"Subject 1 - PC {idx + 1}")
...
# Find the indices of the first three principal components
first_three_indices2 = [0, 1, 2]

# Display the final images based on the first three principal components
plt.figure(figsize=(15, 5))
for i, idx in enumerate(first_three_indices2):
    final_image2 = resultant_matrix2[:, idx].reshape(images_resized2[0].shape)
    plt.subplot(2, 3, i + 1)
    plt.imshow(final_image2, cmap='gray')
    plt.title(f"Subject 2 - PC {idx + 1}")

plt.show()

test_image_path = r'E:\3rd semester\Linear Algebra\Python_Assignment\testimages\testimage2.png'  # Replace with the path to your test image
...
print("loss_sub1",loss_subject1)
print("loss_sub2",loss_subject2)
